...
uint8_t displayInit(I2C_HandleTypeDef *i2c, uint16_t address) {
	...
	//__sendCommand(0xAE);
	//2. Set Osc Frequency
	__sendCommandWithData(0xD5, 0xF0);
	...
	__sendCommandWithData(0xDB, 0x40);
...
static void __setColumn(uint8_t x) {
	uint8_t high = x & 0x0F;
	uint8_t low = ((x>>4) & 0x0F) | 0x10;
	__sendCommand(high);
	__sendCommand(low);
}
static void __setPage(uint8_t page) {
	page |= 0xB0;
	__sendCommand(page);
}
static uint8_t __sendData(uint8_t byte) {
	...
	i2c_buffer[1] = byte;
	return __to_display_transmit(i2c_buffer, 2);
}
static uint8_t __sendDataArray(uint8_t bytes[], uint8_t size) {
	uint8_t i2c_buffer[size+1];
	i2c_buffer[0] = 0x40; //we sending data array
	for (uint8_t i = 1; i != size; i++) {
		i2c_buffer[i] = bytes[i-1];
	}
	return __to_display_transmit(i2c_buffer, size);
}
...
static uint8_t __sendCommandWithData(uint8_t cmd, uint8_t data) {
	...
	i2c_buffer[3] = data;
	return __to_display_transmit(i2c_buffer, 4);
}
static uint8_t __to_display_transmit(uint8_t buff[], uint8_t size) {
	return HAL_I2C_Master_Transmit(_i2c, _address, buff, size, 0x03);
}
